...
- Given the simplicity of the provided data, the medallion architeciture was sufficient for transforming the raw tables into monthly reporting metrics. However, for more complex, real-world data coming from many sources, rather than a medallion architecture, I would implement more layers for greater clarity in the transformation steps (a combination of `ingress`, `raw`, `stage`, `entity`, `pivot`, `reporting`, `export` data layers as needed). 
